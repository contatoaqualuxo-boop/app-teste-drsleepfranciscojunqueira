...
export default function RolesPage() {
  ...
  const [userRolesCount, setUserRolesCount] = useState<Record<string, number>>({});
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        // Fetch user roles to count users per role
    ...
    <main className="min-h-screen bg-[#020617] relative flex overflow-hidden">
      ...
      <div className="flex-1 flex flex-col overflow-y-auto">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-6">
              <div className="flex items-center gap-3">
                ...
                <h1 className="text-white font-medium text-lg">Permissões</h1>
              </div>
            </div>
        ...
        <div className="p-8 space-y-6 flex-1">
          ...
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-2xl font-black text-white">Todas as Permissões</h2>
              <p className="text-blue-100/70 text-sm mt-1">Gerencie as roles/permissões/da/Plataforma/Prévisita</p>
            </div>
          </div>
          ...
          {isLoading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {roles.map((role) => (
                <div key={role.id} className="bg-white/5 border border-white/10 rounded-2xl p-6 hover:bg-white/10 transition-all">
                  <div className="flex items-start gap-4 mb-4">
                    ...
                    <div className="flex-1">
                      <h3 className="text-white font-bold text-lg truncate">{role.name}</h3>
                    </div>
                  ...
                  <div className="flex items-center gap-2 text-white/40 text-xs mt-2">
                    ...
                    <span>
                      {userRolesCount[role.id] || 0} usuário{userRolesCount[role.id] !== 1 ? 's' : ''} vinculado{userRolesCount[role.id] !== 1 ? 's' : ''}
                    </span>
                  </div>
                  <p className="text-white/40 text-xs">
                    Criado em: {new Date(role.created_at).toLocaleDateString('pt-BR', { year: 'numeric', month: 'long', day: 'numeric' })}
                  </p>
                </div>
              </div>
            ))}
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <h3 className="text-white font-bold text-xl mb-2">Nenhuma role/permissão cadastrada</h3>
            <p className="text-blue-100/60 text-sm max-w-md">
              Ainda não há roles/permissões na Plataforma Prévisita. As funcionalidades de criação serão implementadas em breve.
            </p>
          </div>
        )}
      </div>
