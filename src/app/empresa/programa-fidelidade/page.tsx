    ...
    async function fetchData() {
      try {
        ...
        const { data, error } = await supabase
          .from("wallets")
          .select("*, users!left(id, name, email, avatar_url)")
    ...
    <main className="min-h-screen bg-[#020617] relative flex overflow-hidden">
      ...
      <div className="w-72 bg-[#03091c] border-r border-white/10 flex-shrink-0 flex flex-col">
        ...
        <div className="flex-1 p-4 space-y-1">
          ...
          <div className="mb-6">
            ...
            <Link href="/empresa/programa-fidelidade" className="w-full flex items-center gap-3 px-3 py-3 rounded-xl bg-blue-600/20 border border-blue-500/30 text-blue-400 font-medium">
      ...
      <div className="flex-1 flex flex-col overflow-y-auto">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-6">
              <div className="flex items-center gap-3">
                ...
                <h1 className="text-white font-medium text-lg">Programa de Fidelidade</h1>
              </div>
            </div>
        ...
        <div className="p-8 space-y-6 flex-1">
          ...
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-2xl font-black text-white">Programa de Fidelidade</h2>
              <p className="text-blue-100/70 text-sm mt-1">Visualize as carteiras dos clientes e seus saldos</p>
            </div>
          </div>
          ...
          {isLoading ? (
                ...
                <div key={wallet.id} className="bg-white/5 border border-white/10 rounded-2xl p-6 hover:bg-white/10 transition-all">
                  <div className="flex items-start gap-4 mb-4">
                    ...
                    <div className="flex-1">
                      <h3 className="text-white font-bold text-lg truncate">{wallet.users?.name || "Cliente"}</h3>
                      {wallet.users?.email && (
                        <p className="text-blue-200/70 text-xs truncate">{wallet.users.email}</p>
                      )}
                    </div>
                  </div>
                  <div className="grid grid-cols-3 gap-3">
                    {wallet.points_balance !== null && (
                      <div className="p-3 bg-amber-500/10 rounded-xl border border-amber-500/20 flex flex-col items-center">
                        <Coins className="w-4 h-4 text-amber-400 mb-1" />
                        <span className="text-white font-bold text-base">{wallet.points_balance.toLocaleString()}</span>
                        <span className="text-amber-400 text-[10px]">Pontos</span>
                      </div>
                    )}
                    {wallet.credits_balance !== null && (
                      <div className="p-3 bg-blue-500/10 rounded-xl border border-blue-500/20 flex flex-col items-center">
                        <CreditCard className="w-4 h-4 text-blue-400 mb-1" />
                        <span className="text-white font-bold text-base">{wallet.credits_balance.toLocaleString()}</span>
                        <span className="text-blue-400 text-[10px]">Créditos</span>
                      </div>
                    )}
                    {wallet.cashback_balance !== null && (
                      <div className="p-3 bg-emerald-500/10 rounded-xl border border-emerald-500/20 flex flex-col items-center">
                        <Gift className="w-4 h-4 text-emerald-400 mb-1" />
                        <span className="text-white font-bold text-base">R$ {wallet.cashback_balance.toFixed(2)}</span>
                        <span className="text-emerald-400 text-[10px]">Cashback</span>
                      </div>
                    )}
                  </div>
                  <p className="text-white/40 text-xs mt-4">
                    Criada em: {new Date(wallet.created_at).toLocaleDateString('pt-BR', { year: 'numeric', month: 'long', day: 'numeric' })}
                  </p>
                </div>
              ))}
            </div>
            ...
            <div className="flex flex-col items-center justify-center py-16 text-center">
              <Gift className="w-20 h-20 text-white/10 mb-4" />
              <h3 className="text-white font-bold text-xl mb-2">Nenhuma carteira cadastrada</h3>
              <p className="text-blue-100/60 text-sm max-w-md">
                Ainda não há carteiras de clientes no programa de fidelidade. As funcionalidades de gestão serão implementadas em breve.
              </p>
            </div>
          )}
        </div>
