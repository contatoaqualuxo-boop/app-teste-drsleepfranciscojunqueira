...
CREATE TABLE IF NOT EXISTS notification_templates (
  ...
  name TEXT NOT NULL,
  slug TEXT NOT NULL,
  type TEXT NOT NULL DEFAULT 'email', -- email, sms, whatsapp, push
  subject TEXT,
  body TEXT NOT NULL,
  variables TEXT[], -- lista de variáveis disponíveis, ex: ['user_name', 'product_name', 'rotation_date']
  is_active BOOLEAN DEFAULT true,
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW(),
...
async function main() {
  ...
  // Executar o SQL dividido em batches (para evitar erros de tamanho)
  const statements = sql.split(';').map(s => s.trim()).filter(s => s.length > 0);

  console.log(`Total de ${statements.length} instruções SQL para executar.`);
  ...
  for (const [index, statement] of statements.entries()) {
    ...
    try {
      ...
      if (error) {
        console.error(`Erro na instrução ${index + 1}:`, error.message);
        errorCount++;
      } else {
        successCount++;
      }
    } catch (err) {
      console.error(`Erro na instrução ${index + 1}:`, (err as Error).message);
      errorCount++;
    }
  }
